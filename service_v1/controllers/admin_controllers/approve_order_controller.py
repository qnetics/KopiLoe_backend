...
def approve_order_controller (header_request, body_request) :
        ...
        if not expired_checker(access_token = access_token) :
            ...
            if token_checker(access_token = access_token) :
                ...
                if admin_checker (access_token = access_token) :
                    http_status_code, message_response, data_response = approve_order (body_request)
...
def approve_order (body_request) : 
    ...
    if order_model.objects.filter(
        ...
        income : int = product_id.product_price * order_data.order_quantity

        if not resource_model.objects.filter(id = 1).exists() :
            resource_model.objects.create(incomes = 0)

        resource_data : resource_model = resource_model.objects.get(id = 1)

        resource_data.incomes : int = resource_data.incomes + income
        resource_data.save()

        order_data.delete()

    message_response : str  = "berhasil menyelesaikan pesanan"
