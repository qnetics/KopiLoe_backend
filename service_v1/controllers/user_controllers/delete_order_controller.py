        ...
        if not expired_checker(access_token = access_token) :
            ...
            if token_checker(access_token = access_token) :
                ...
                if not admin_checker (access_token = access_token) :
                    http_status_code, message_response, data_response = delete_order (body_request, access_token)
...
def delete_order (body_request, access_token) : 

    if body_request.get('product_url') == (None or ""):
