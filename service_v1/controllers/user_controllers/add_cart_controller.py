...
def add_cart_controller (header_request, body_request) :
...
def add_cart (access_token, body_request) : 
    ...
    if cart_model.objects.filter(
        ...
        cart_user = cart_model.objects.create(
            user = user_auth_model.objects.get(
                access_token = access_token).user,
                
            product = product_model.objects.get(product_url = body_request.get("product_url")),

            order_quantity = body_request.get("order_quantity")
        )

    message_response : str = "barang telah ditambahkan"
    http_status_code : int = 201
